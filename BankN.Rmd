...
# temp <- ifelse(temp >= Sys.Date(), temp - 365, temp)
# class(temp) <- "Date"
temp
...
```{r}
head(bankn[c(9,10,14)], n = 5)
```

# Question 6

## Question 6 | Plot time-series graph(s) to show comparison between deposits made over 2 comparative periods
within the data (month/quarter).
...
bankn <- bankn[, colnames(bankn)[c(1:10,15,11:14)]]
```

## Create Time Series

Using <strong>tseries</strong> library:
```{r}
library(forecast)
library(tseries)
library(tidyquant)
...
bankn <- bankn[!is.na(bankn$deposit) | bankn$deposit_date == "NA",]
# ----------------------------------------------
