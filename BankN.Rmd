...
```{r}
bankn$education <- droplevels(bankn$education)
summary(bankn$education)
```
...
## Data Cleaning and Observation | Housing
...
## Data Cleaning and Observation | Loan
...
##  Data Cleaning and Observation | Contact
...
## Data Cleaning and Observation | Day
...
## Data Cleaning and Observation | Month
...
* Value Range : "january" to "december"

* Min : month(s) with lowest frequency 

* Max : moth(s) with highest frequency

## Data Cleaning and Observation | Month

```{r , fig.align='center'}
barplot(prop.table(table(bankn$month)))
```

## Data Cleaning and Observation | Month

```{r, fig.align='center'}
dates_noise <- bankn[((bankn$month == 'january' & bankn$day > 31) | 
                        (bankn$month == 'february' & bankn$day > 28) |
                        (bankn$month == 'march' & bankn$day > 31) |
                        (bankn$month == 'april' & bankn$day > 30) |
                        (bankn$month == 'may' & bankn$day > 31) |
                        (bankn$month == 'june' & bankn$day > 30) |
                        (bankn$month == 'july' & bankn$day > 31) |
                        (bankn$month == 'august' & bankn$day > 31) |
                        (bankn$month == 'september' & bankn$day > 30) |
                        (bankn$month == 'october' & bankn$day > 31) |
                        (bankn$month == 'november' & bankn$day > 30) |
                        (bankn$month == 'december' & bankn$day > 31) 
...
* Noise: Out of range dates

* outlies: None

####  Fixing Errors/Unknown Values | DAY & MONTH
```{r}
...
# viewing data post fix
```

## Data Cleaning and Observation | Duration
...
* Min : shortest call druation (seconds)

* Max : longest call duration (seconds)
...
## Data Cleaning and Observation | Deposit

- Deposit feature stores the amount of money deposited by a customer during their last transaction.

- Doing a summary of deposit reveals:
  - values range from 0 to 44257o- 25% of the values  are between 0 - 1415(first quartile)
  - the median value is 8817
  - the mean is 16220
  - 75% of the values are between 0-119169 (third quartile)
  - the max is 442570
  - there are 152 missing values
----
```{r}
...
#### Fixing Missing Values and Zeros in Deposit. 

- In order to fix them, central tendency is used.
- Missing values are replaced with the mean of deposit.
- The mean is used because the data is skewed.
----
```{r}
...
----
```{r}
kable(head(bankn[is.na(bankn$deposit),],n = 4))
```


## Data Cleaning and Observation | Deposit

#### Fixing  noise : Missing Values and Zeros in Deposit. 

- Individuals were grouped by their jobs.
- Missing values and zeros within each category were replaced with the mean of that category.
...
- Doing summary reveals there are that all missing values and zeros have been replaced.
...
## Data Cleaning and Observation | Balance
...
* Range of value: -167760 to 10537920

* Min : The account containing the lowest amount 

* Max : The account containing the largest amount 
...
## Summary of Data | Balance
>>>>>>> f79ceb213d0c0876c8c546628a21f2e9b42d2a45
...
# Data Transformation | Convert the duration from the current value in seconds to minutes.
...
#### After
```{r}
kable(bankn[1:3,c("RefNum","duration")], n = 3)
```
...
# Data Transformation | Normalizing Deposits
...
# Data Transformation | Number of Days Since the Last Deposit

## Data Transformation | Number of Days Since the Last Deposit

## Data Transformation | What was done?
...
# Data Analysis | What is the dominant educational level of each category of job?
...
```{r}
barplot(prop.table(table(jobs_admin$education)))
```
...
```{r}
barplot(prop.table(table(jobs_blue_collar$education)))
```
...
```{r}
barplot(prop.table(table(jobs_entrepreneur$education)))
```
...
```{r}
barplot(prop.table(table(jobs_housemaid$education)))
```
...
```{r}
barplot(prop.table(table(jobs_management$education)))
```
...
```{r}
barplot(prop.table(table(jobs_retired$education)))
```
...
```{r}
barplot(prop.table(table(jobs_self_employed$education)))
```
...
```{r}
barplot(prop.table(table(jobs_services$education)))
```
...
```{r}
barplot(prop.table(table(jobs_technician$education)))
```
...
```{r}
barplot(prop.table(table(jobs_unemployed$education)))
```
...
# Data Analysis | Is there a correlation between deposit and balance? Discuss the findings. Explain the implications.
...
# Data Analysis | What percentage of customers who are married also are in overdraft?
...
# Data Analysis | Propose metric to determine customers with least credit risk.
