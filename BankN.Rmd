...
## Deposits
  ...
  Consider persons in the 3rd Quartile, which is `r summary(bankn$deposits)["3rd Qu."]`
```{r}
hist(bankn$deposit, breaks=50)
```

## Last Deposit

  Consider persons who have only deposited in the final quarter of the year. These persons would perhaps be depositing more frequently, which could indicate a steady income.
  
```{r}
fourth <- bankn[bankn$month %in% c('october','november','december'),]
```
...
first <- risk_scores$deposit <= summary(risk_scores$deposit)["1st Qu."]
risk_scores$deposit <- ifelse(risk_scores$deposit <= summary())
```
