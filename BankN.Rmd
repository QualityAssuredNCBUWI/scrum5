...
bankn_missing_balace <- bankn[is.na(bankn$balance),]

for(i in 1:nrow(bankn_missing_balace)){
  bankn_missing_balace[i,'balance'] <- median_list[median_list$Group.1 == (bankn_missing_balace[i,'job']),'x']
}

bankn[is.na(bankn$balance),] = bankn_missing_balace
```
...
```{r}
cor.test(bankn$deposit,bankn$balance)
```
