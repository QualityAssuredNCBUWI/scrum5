...
```{r}
SUM(bankn[bankn$month %in% c('january','february','march'),"deposit"])
SUM(bankn[bankn$month %in% c('april','may','june'),"deposit"])
SUM(bankn[bankn$month %in% c('july','august','september'),"deposit"])
SUM(bankn[bankn$month %in% c('october','november','december'),"deposit"])
...
```{r}
plot.xts(deposits.xts[months(index(deposits.xts)) %in% month.name[1:3]])
```
