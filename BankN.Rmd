...
for(i in 1:nrow(bankn)){
  if(bankn[i,'deposit'] > upper_limit){
    bankn[i,'deposit'] <- upper_limit
  }
  
  if(bankn[i,'deposit'] < lower_limit){
    bankn[i,'deposit'] <- lower_limit
  }
}
```
...
>>>>>>> 0b9ef1d6e48ee8db2b5ef18a31dc6d4bbc809b09
...
for(i in 1:nrow(bankn)){
  if(bankn[i,'balance'] > upper_limit){
    bankn[i,'balance'] <- upper_limit
  }
  
  if(bankn[i,'balance'] < lower_limit){
    bankn[i,'balance'] <- lower_limit
  }
}
...
plot.xts(deposits.xts[months(index(deposits.xts)) %in% month.name[7:9]], main = "", col = "red")
```
